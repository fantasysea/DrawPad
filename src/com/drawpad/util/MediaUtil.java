...
public class MediaUtil {
		...
		if (checkSDCard() && null != context) {
			...
			if (null != c) {
				...
				for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
					...
					videoInfo.videoUri = Uri.withAppendedPath(baseUri, String
							.valueOf(videoInfo.id));
					videoInfo.videoThumb = MediaStore.Video.Thumbnails
							.getThumbnail(cr, videoInfo.id,
		...
		if (checkSDCard() && null != context) {
			...
			if (null != c) {
				...
				for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
					...
					imageInfo.imageUri = Uri.withAppendedPath(baseUri, String
							.valueOf(imageInfo.id));
					imageInfo.imageThumb = MediaStore.Images.Thumbnails
							.getThumbnail(cr, imageInfo.id,
	...
	public List<String> getImagesFromPath(String path) {
	...
	private boolean isImageFile(String path) {
		return path.endsWith(".png") ? true : false;
	}
	...
	protected class MediaUtilHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			if (MediaUtil.GET_VIDEOS_MESSAGE == msg.what) {
				if (null != mediaUtil.m_callback
						&& !mediaUtil.m_isGetVideoCancelled) {
				...
				if (null != mediaUtil.m_callback
						&& !mediaUtil.m_isGetImageCancelled) {
	...
	protected class MediaUtilWorker implements Runnable {
		...
		public void run() {
			...
			if (null != handler) {
				if (m_isGetVideo) {
					List<VideoInfo> videos = MediaUtil.this
							.getVideosFromSDCard(MediaUtil.this.m_context);
					...
					List<ImageInfo> images = MediaUtil.this
							.getImagesFromSDCard(MediaUtil.this.m_context);
	...
	public class VideoInfo {
		...
		public void formatValues() {
			...
			final Bitmap bmp = BitmapUtil.getSizedBitmap(videoThumb,
	...
	public class ImageInfo {
		...
		public void formatValues() {
			...
			final Bitmap bmp = BitmapUtil.getSizedBitmap(imageThumb,
					m_thumbnailWidth, m_thumbnailHeight);
